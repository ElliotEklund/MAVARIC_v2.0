#include "auto_correlation_nrpmd.hpp"

auto_correlation_nrpmd::auto_correlation_nrpmd(int my_id,int num_procs, int root_proc)
    :my_id(my_id),
     num_procs(num_procs),
     root_proc(root_proc)
{
...
int auto_correlation_nrpmd::compute(unsigned long long num_trajs_global,
    ...
    /* Objects used to calculated MVRPMD Sign*/
    ...
    csrpmd_forces F(nuc_beads, elec_beads, num_states, mass,
    ...
    /* HACK ALERT! This should be cleaned up later*/
...
double auto_correlation_nrpmd::compute_centroid(const vector<double> &Q){
    double centroid = sum(Q);
    return centroid/nuc_beads;
}
void auto_correlation_nrpmd::set_system(int nuc_beadsIN, int elec_beadsIN, int num_statesIN,
...
void auto_correlation_nrpmd::request_calcs(bool pacIN, int pac_strideIN, bool bpIN,
...
void auto_correlation_nrpmd::set_time(double dtIN, double total_timeIN){
    dt = dtIN;
